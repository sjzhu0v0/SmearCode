...
SmearEvent::SmearEvent(bool isRead, string fileName) {
  if (!isRead) {
    ...
    tree_event->Branch("event_id", &event_id, "event_id/I");
...
void SmearEvent::McTreeInitializing(TTree *tree) {
  tree_mcevent = tree;
  tree_mcevent->SetBranchAddress("event", &mc_event);
}
...
SmearEvent::CalCut_D0(int no1, int no2) // calculate cuts no1: pion, no2:kaon
{
  ...
  tempStr.dca_12 = trackdca_cal(pionStr.p_smeared, kaonStr.p_smeared, c_pion,
                                c_kaon, &c_D0_dec);
  ...
  tempStr.num_final_particles = num_final_particles;

  tree_mcevent->GetEntry(event_id);

  tempStr.num_mc_final_charged_particles = 0;
  for (int iTrack = 0; iTrack < mc_event->GetNTracks(); iTrack++) {
    const erhic::ParticleMC *mcParticle = mc_event->GetTrack(iTrack);
    if (abs(mcParticle->id) == 11 || abs(mcParticle->id) == 13 ||
        abs(mcParticle->id) == 211 || abs(mcParticle->id) == 321 ||
        abs(mcParticle->id) == 2212) {
      if (mcParticle->pt > 0.3 && abs(mcParticle->eta) < 3.5) {
        tempStr.num_mc_final_charged_particles++;
      }
    }
  }
  return tempStr;
...
SmearEvent::CalCut_Jpsi(int no1, int no2) // calculate cuts no1: pion, no2:kaon
{
  ...
  tempStr.dca_12 = trackdca_cal(elecStr.p_smeared, posiStr.p_smeared, c_elec,
                                c_posi, &c_Jpsi_dec);
    ...
    int no3) // calculate cuts no1: pion, no2:kaon, no3:proton
{
  ...
  TVector3 c_temp1, c_temp2, c_temp3;
  double dca_daughters_pion_kaon = trackdca_cal(
      pionStr.p_smeared, kaonStr.p_smeared, c_pion, c_kaon, &c_temp1);
  double dca_daughters_proton_kaon = trackdca_cal(
      protonStr.p_smeared, kaonStr.p_smeared, c_proton, c_kaon, &c_temp2);
  double dca_daughters_proton_pion = trackdca_cal(
      protonStr.p_smeared, pionStr.p_smeared, c_proton, c_pion, &c_temp3);
  c_lambda_c_dec = c_temp1 + c_temp2 + c_temp3;
  ...
  if (dca_daughters_pion_kaon > dca_daughters_proton_kaon) {
    tempStr.maxDca_daughters = dca_daughters_pion_kaon;
  } else {
    tempStr.maxDca_daughters = dca_daughters_proton_kaon;
  }
  if (tempStr.maxDca_daughters < dca_daughters_proton_pion) {
