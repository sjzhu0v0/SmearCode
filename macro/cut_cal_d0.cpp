...
smear_event.McTreeInitializing(t_event);
  ...
  t_cut->Branch("num_final_particles", &cutstr_d0.num_final_particles, "num_final_particles/I");
  t_cut->Branch("num_mc_final_charged_particles", &cutstr_d0.num_mc_final_charged_particles, "num_mc_final_charged_particles/I");
  ...
  for (long iEvent = 0; iEvent < smear_event.GetEntries(); iEvent++) {
    ...
    for (int iPion = 0; iPion < smear_event.num_final_particles; iPion++) {
      for (int iKaon = 0; iKaon < smear_event.num_final_particles; iKaon++) {
        ...
        if (pid_system == 0) {
          right_signs = smear_event.bool4D0Cut(SmearEvent::ECCE_PID,2.,iPion,iKaon);
        } else if (pid_system == 1) {
          right_signs = smear_event.bool4D0Cut(SmearEvent::ECCE_TOF_EMC,2.,iPion,iKaon);
        } else if (pid_system == 2) {
          right_signs = smear_event.bool4D0Cut(SmearEvent::ECCE_CHERENKOV_EMC,2.,iPion,iKaon);
        } else if (pid_system == 3) {
          right_signs = smear_event.bool4D0Cut(SmearEvent::EICC_ACCEPTANCE,2.,iPion,iKaon);
        } else if (pid_system == 4) {
          right_signs = smear_event.bool4D0Cut(SmearEvent::PERFECT_PID,2.,iPion,iKaon);
        } else if (pid_system == 5) {
          right_signs = smear_event.bool4D0Cut(SmearEvent::NO_PID,2.,iPion,iKaon);
        }
        if (right_signs) {
...
int main(int argc, char* argv[]) {
  ...
  cut_cal_d0(name_input, atoi(argv[2]), argv[3], argv[4]);
  // cut_cal_d0("~/HIUser_sjzhu/work3/code/Smear_Code/bin/d0_decay_test.root" ,3, "cut_d0_test.root" ,"/ustcfs/HICUser/ephy/work/sjzhu/Singularity-master/cvmfs/work/pythia6_generator/3.5_20/generator1/mc.root");
  return 0;
}
